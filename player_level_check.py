...
for name in l_name.split(' '):
    ...
    response = requests.request("GET"
                                , "https://zwfw.sport.gov.cn/level.do?m=getLevelList&item=36.1&name={}".format(name)
                                , headers=headers)
    ...
    for list_data in response.json()['data']['data']['list_data']:
        ...
        payload = "{\"certificateNo\":\"{}\",\"athleteRealName\":\"{}\"}".format(list_data['certificate_num'], name)
        response = requests.request("POST"
                                    , url="https://ydydj.univsport.com/api/system/athlete/front-end-list"
                                    , headers=headers
                                    , data=payload)
        # 获取运动员信息详情
        payload = "{\"athleteInfoId\":\"{}\"}".format(response.json()['data']['list'][0]['athleteInfoId'])
        response = requests.request("POST"
        ...
        athlete_near_pic = "https://ydydj.univsport.com/{}".format(response.json()['data']['oneInchPhoto'])  # 照片
